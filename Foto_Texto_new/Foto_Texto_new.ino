...
// Las credenciales ahora vienen de secrets.h
...
String leerRespuestaHTTP(WiFiClientSecure &client);
...
void setup() {
  ...
  if (err != ESP_OK) {
    ...
    Serial.print("Error iniciando la camara: 0x");
  ...
  String response = "Dispositivo listo";
...
void loop() {
  ...
  if (buttonState == LOW) {
    ...
    String response = SendStillToGeminiVision(
      gemini_Key,
      gemini_Chat
    );
...
String leerRespuestaHTTP(
  ...
  // ======================================================
    ...
    while (
      recibidos < contentLength
      &&
      millis() - ultimoDato < 15000
    ) {
